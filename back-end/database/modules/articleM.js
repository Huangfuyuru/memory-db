...
async function addarticle(text){
    let sql = 'insert into article(imgurl,content,tag,style,uid) VALUES($1,$2,$3,$4,$5);';
    let ret = await pgdb.query(sql,[text.imgurl,text.content,text.tag,text.style,text.uid]);
    if(ret.rowCount<=0){
        return 1
    }else{
        return 0;
